...
func TemplateEntity(s util.StructDef, v string) (string, error) {
	...
	template += `return errors.New("missing operator set for field type: " + typ)` + "\n"
	template += "}\n"
	template += "}\n\n"
	...
	template += `return errors.New("failed to encode type metadata: " + err.Error())` + "\n"
	template += "}\n"
	template += "if err := x.Decode(); err != nil {\n"
	template += `return errors.New("failed to decode type metadata: " + err.Error())` + "\n"
	template += "}\n"
	...
	template += "mu.Lock()\n"
	template += "defer mu.Unlock()\n\n"
	template += "targetEntity, ok := target.(*" + s.Name + ")\n"
	template += "if !ok {\n"
	template += "return false\n"
	template += "}\n\n"
	template += "for _, instance := range Mem {\n"
	template += "// If both UUIDs are nil/zero, fall back to pointer comparison\n"
	template += "if (instance.Uuid == uuid.Nil && targetEntity.Uuid == uuid.Nil) {\n"
	template += "if instance == targetEntity {\n"
	template += "return true\n"
	template += "}\n"
	template += "} else if instance.Uuid == targetEntity.Uuid && instance.Uuid != uuid.Nil {\n"
	template += "// For entities with actual UUIDs, compare by UUID\n"
	template += "return true\n"
	template += "}\n"
	template += "}\n"
	template += "return false\n"
	...
	template += "resp, err := c.SendReceive(msg)\n"
	template += "if err != nil {\n"
	template += "return err\n"
	template += "}\n"
	...
	template += "func DbDeleteAll(c *hconn.HConn) error {\n"
	template += "entities, err := DbGetAll(c)\n"
	template += "if err != nil {\n"
	template += "return err\n"
	template += "}\n\n"
	template += "for _, entity := range entities {\n"
	template += "if err := entity.DbDelete(c); err != nil {\n"
	template += "return err\n"
	template += "}\n"
	template += "}\n\n"
	template += "return nil\n"
	...
	template += "if item.Uuid == target.Uuid {\n"
